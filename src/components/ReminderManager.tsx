...
export function ReminderManager() {
  ...
  React.useEffect(() => {
    ...
    return () => window.removeEventListener('trigger-test-alarm', handleTestAlarm);
  }, []);
  ...
  const handleCall = (person: Person) => { window.location.href = `tel:${person.phone}`; handleDismiss(person); };
            ...
            <div className="flex flex-col items-center space-y-4">
              <div className="relative">
                ...
                <div className="relative bg-primary p-6 rounded-full shadow-2xl border-4 border-primary/20">
                  <Bell className="h-10 w-10 text-primary-foreground animate-bounce" />
                </div>
              </div>
              <div className="text-center space-y-1">
                <h2 className="text-3xl font-black text-foreground tracking-tighter uppercase leading-none">FOLLOW-UP DUE</h2>
                <p className="text-[10px] text-primary font-black uppercase tracking-[0.4em]">Active outreach task</p>
              </div>
            </div>

            <div className="w-full max-w-sm bg-popover border border-border rounded-[3rem] shadow-2xl p-10 space-y-8 relative overflow-hidden">
              <div className="flex flex-col items-center text-center space-y-4 relative z-10">
                <Avatar className="h-32 w-32 border-4 border-primary/30 shadow-2xl rounded-[2.5rem]">
                  ...
                  <AvatarFallback className="text-3xl font-black bg-muted text-primary">{activeAlarm.fullName.charAt(0)}</AvatarFallback>
                </Avatar>
                <div className="space-y-1.5">
                  <h3 className="text-3xl font-black text-foreground tracking-tight leading-none uppercase truncate max-w-[280px]">{activeAlarm.fullName}</h3>
                  <p className="text-base font-black text-primary/80 tracking-widest">{activeAlarm.phone}</p>
                </div>
              </div>
              {activeAlarm.lastCallRemark && (
                <div className="bg-muted p-6 rounded-3xl border border-border space-y-2">
                  <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-muted-foreground"><MessageSquare className="h-3 w-3" /> Latest Context</div>
                  <p className="text-xs text-foreground/80 font-bold leading-relaxed italic">"{activeAlarm.lastCallRemark}"</p>
                </div>
              )}
            </div>

            <div className="w-full max-w-sm grid grid-cols-1 gap-4">
              <Button className="w-full h-20 text-xl font-black rounded-3xl shadow-2xl shadow-primary/30 uppercase tracking-tight bg-primary text-primary-foreground" onClick={() => handleCall(activeAlarm)}>
                <Phone className="mr-4 h-8 w-8" /> MAKE THE CALL
              </Button>
              <div className="grid grid-cols-2 gap-4">
                <Button variant="outline" className="h-16 text-[10px] font-black uppercase tracking-widest border-border text-foreground bg-muted/50 rounded-2xl" onClick={() => handleSnooze(activeAlarm)}>Snooze 10m</Button>
                <Button variant="ghost" className="h-16 text-[10px] font-black uppercase tracking-widest text-destructive hover:text-destructive rounded-2xl" onClick={() => handleDismiss(activeAlarm)}><X className="mr-2 h-4 w-4" /> Dismiss</Button>
              </div>
            </div>
