...
import { Loader2, CheckCircle2, Phone, AlertCircle, CalendarCheck2, UserPlus, ArrowRight } from 'lucide-react';
...
export default function CheckInClient({ groupId }: { groupId: string }) {
  ...
  if (!group) return (
    <div className="min-h-screen flex items-center justify-center bg-[#11121d] p-4">
      <Card className="max-w-md w-full text-center p-8 bg-[#1e1e2e] border-none rounded-[2.5rem]">
        <AlertCircle className="h-12 w-12 text-destructive mx-auto mb-4" />
        <CardTitle className="text-white">Invalid Outreach Link</CardTitle>
      </Card>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#11121d] flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md space-y-12">
        <div className="flex flex-col items-center gap-6 animate-in fade-in duration-700">
          ...
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-black text-white tracking-tighter uppercase leading-none">JOIN THE GROUP</h1>
            <p className="text-[10px] text-[#FF9800] font-black uppercase tracking-[0.4em]">{group.name}</p>
          </div>
        </div>

        {status === 'idle' && (
          <Card className="shadow-2xl rounded-[3rem] border-none bg-[#1e1e2e] overflow-hidden animate-in zoom-in-95 duration-500">
            <CardHeader className="text-center pt-10 pb-4">
                <CardTitle className="text-2xl font-black text-white uppercase tracking-tight">ATTENDANCE LOG</CardTitle>
            </CardHeader>
            <CardContent className="p-10 pt-4">
                <form onSubmit={handleCheckIn} className="space-y-8">
                    <div className="space-y-3 text-center">
                        <Label className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em]">Enter Registered Phone</Label>
                        <div className="relative">
                            <Phone className="absolute left-5 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500" />
                            <Input placeholder="10-digit mobile" className="pl-14 h-16 text-2xl font-black rounded-2xl border-white/5 bg-[#161623] text-white focus:ring-[#FF9800]" value={phone} onChange={e => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))} type="tel" required />
                        </div>
                    </div>
        ...
        {status === 'success' && (
          <Card className="shadow-2xl rounded-[3rem] border-none bg-[#1e1e2e] overflow-hidden text-center p-12 space-y-8 animate-in zoom-in-95 duration-500">
             ...
             <div className="space-y-3">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-[#929DD8] font-black text-xl uppercase tracking-tight">{foundPerson?.fullName}</p>
                </div>
             </div>
        ...
        {status === 'not-found' && (
           <Card className="shadow-2xl rounded-[3rem] border-none bg-[#1e1e2e] overflow-hidden p-12 text-center space-y-10 animate-in zoom-in-95 duration-500">
              <div className="mx-auto bg-amber-500/10 p-8 rounded-[2.5rem] w-fit border border-amber-500/20">
                 <UserPlus className="h-16 w-16 text-amber-500" />
              </div>
              <div className="space-y-3">
                 <h2 className="text-2xl font-black text-white uppercase tracking-tight">NEW TO CRM?</h2>
                 <p className="text-sm text-slate-400 font-bold leading-relaxed px-2">
                    We couldn't find a profile for <span className="text-[#FF9800] font-black">{phone}</span>.<br/>Register now to join the list!
                 </p>
              </div>
              
              <div className="space-y-4">
                  <div className="bg-[#161623] p-5 rounded-2xl border border-white/5 flex items-center justify-between">
                      <div className="text-left">
                          <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Coordinator</p>
                          <p className="text-sm font-black text-white">{groupOwner?.name || 'App Team'}</p>
                      </div>
                  ...
                  <Button onClick={goToRegister} className="w-full h-16 text-lg font-black rounded-2xl shadow-xl bg-primary hover:bg-primary/90 text-white transition-all hover:scale-[1.01]">
      ...
      <footer className="mt-16 opacity-30">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em]">FOLK SPIRITUAL GEMS</p>
      </footer>
    </div>
  );
}
