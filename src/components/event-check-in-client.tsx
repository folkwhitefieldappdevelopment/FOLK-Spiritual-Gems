...
export default function EventCheckInClient({ groupId, eventId }: { groupId: string, eventId: string }) {
  ...
  // Assignment states for new users
  ...
  const goToRegister = () => {
      ...
      router.push(`/register/?id=${selectedGuideId}&groupId=${groupId}&eventId=${eventId}${enablerPart}&phone=${phone}`);
  };
      ...
      <div className="w-full max-w-md space-y-12">
        <div className="flex flex-col items-center gap-6 animate-in fade-in duration-700">
          ...
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-black text-white tracking-tighter uppercase leading-none">JOIN THE EVENT</h1>
            ...
            <CardContent className="p-10 pt-4">
                <form onSubmit={handleCheckIn} className="space-y-8">
                    ...
                    <Button className="w-full h-16 text-lg font-black rounded-2xl shadow-xl bg-[#FF9800] hover:bg-[#F57C00] text-black transition-all hover:scale-[1.01]" disabled={isSubmitting || phone.length < 10}>
                        {isSubmitting ? <Loader2 className="h-6 w-6 animate-spin mr-2" /> : "CONTINUE"}
                    </Button>
                </form>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-2xl rounded-[3rem] border-none bg-[#1e1e2e] overflow-hidden text-center p-12 space-y-8 animate-in zoom-in-95 duration-500">
             ...
             <div className="space-y-3">
                <h2 className="text-3xl font-black text-white uppercase tracking-tighter">
                    {statusMsg.includes('already') ? 'LOGGED PREVIOUSLY' : 'ALL DONE! ✨'}
                </h2>
                <div className="space-y-1">
                    <p className="text-slate-400 font-bold">
                        {statusMsg.includes('already') ? "You already submitted this form." : "Thank you for joining the session."}
                    </p>
        ...
        {status === 'assignment' && (
           <Card className="shadow-2xl rounded-[3rem] border-none bg-[#1e1e2e] overflow-hidden p-10 text-center space-y-8 animate-in zoom-in-95 duration-500">
              <div className="mx-auto bg-amber-500/10 p-8 rounded-[2.5rem] w-fit border border-amber-500/20 shadow-inner">
                 <UserPlus className="h-12 w-12 text-amber-500" />
              </div>
              <div className="space-y-3">
                 <h2 className="text-2xl font-black text-white uppercase tracking-tight">NEW MEMBER?</h2>
                 <p className="text-sm text-slate-400 font-bold leading-relaxed">
                    We couldn't find a record for <span className="text-[#FF9800] font-black">{phone}</span>.<br/>Select your coordinator to register.
                 </p>
              </div>
              
              <div className="space-y-4 text-left">
                  <div className="space-y-2">
                      <Label className="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">Select Folk Guide</Label>
                  ...
                  <div className="space-y-2">
                      <Label className="text-[9px] font-black uppercase text-slate-500 tracking-widest ml-1">Select Enabler</Label>
              ...
              <Button onClick={goToRegister} className="w-full h-16 text-lg font-black rounded-2xl shadow-xl bg-[#FF9800] text-black transition-all hover:scale-[1.01]" disabled={!selectedEnablerId}>
