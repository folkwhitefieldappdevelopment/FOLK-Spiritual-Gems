...
import { bulkUpdatePeopleFields, assignCoEnablerToPeople } from '@/services/people-service';
...
export function BulkEditPersonDialog({
  ...
  const [isCoEnablerManagerOpen, setIsCoEnablerManagerOpen] = React.useState(false);
  ...
  return (
    <>
                ...
                <Accordion type="multiple" className="space-y-4">
                  ...
                  <AccordionItem value="co-enabler" className="border-none">
                    ...
                    <AccordionContent className="p-6 bg-muted/20 border border-t-0 border-border rounded-b-2xl space-y-4">
                      <Button 
                        variant="outline" 
                        className="w-full h-12 rounded-xl border-primary/20 text-primary hover:bg-primary/5 font-black uppercase text-[10px] tracking-widest"
                        onClick={() => setIsCoEnablerManagerOpen(true)}
                      >
                      ...
                      <div className="relative">
                        ...
                        <div className="relative flex justify-center text-[9px] uppercase font-black text-muted-foreground"><span className="bg-muted px-2">Or Direct Field Update</span></div>
                      </div>
      ...
      <AssignCoEnablerDialog
        isOpen={isCoEnablerManagerOpen}
        setIsOpen={setIsCoEnablerManagerOpen}
        onSave={async (u) => {
          if (u && appUser) {
            await assignCoEnablerToPeople(selectedIds, u, { id: appUser.id, name: appUser.name, role: appUser.role });
            toast({ title: 'Co-Enabler Assigned' });
            onSuccess();
          }
        }}
        peopleCount={selectedIds.length}
