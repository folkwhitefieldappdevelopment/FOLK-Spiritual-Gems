...
export default function GroupsPage() {
  ...
  const [userFilterId, setUserFilterId] = React.useState('all');
  ...
  const { userFilterOptions } = React.useMemo(() => {
    ...
    return { userFilterOptions: { guides, enablers } };
  }, [allUsers]);
  ...
  const filteredGroups = React.useMemo(() => {
    ...
    const staticGroups = baseGroups.filter(g => !g.isDynamic).map(group => {
        ...
        return { ...group, filteredMemberCount: filteredMembers.length };
    ...
    if (searchTerm.trim()) {
      ...
      return combined.filter(g => g.name.toLowerCase().includes(lowercasedTerm) || (g.description || '').toLowerCase().includes(lowercasedTerm));
    }
    ...
    return (
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filteredGroups.map((group) => {
          ...
          const memberCount = group.isDynamic ? group.memberCount : group.filteredMemberCount ?? group.memberCount;
      ...
      <SelectContent>
        ...
        {userFilterOptions.guides.length > 0 && (
          <UiSelectGroup>
            ...
            {userFilterOptions.guides.map(u => <SelectItem key={u.id} value={u.id}>{u.name} ({u.fgCode || 'N/A'})</SelectItem>)}
          </UiSelectGroup>
        )}
        {userFilterOptions.enablers.length > 0 && (
          <UiSelectGroup>
            <SelectLabel>Folk Enablers</SelectLabel>
            {userFilterOptions.enablers.map(u => <SelectItem key={u.id} value={u.id}>{u.name}</SelectItem>)}
          </UiSelectGroup>
        )}
      </SelectContent>
    </Select>
