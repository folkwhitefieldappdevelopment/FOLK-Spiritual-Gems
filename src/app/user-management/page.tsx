  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
        ...
        toast({
            ...
            description: 'The user database record has been removed. Note: Authentication account must be removed manually in Firebase Console.'
        });
        fetchUsersAndGuides();
    } catch (error) {
  ...
  const handleSendPasswordReset = async (email: string) => {
    try {
        ...
        toast({
            variant: 'destructive',
            title: 'Error',
            description: 'Could not send reset email. Check if the user exists.',
        });
    }
  };
  ...
  const handleUpdateUser = async (data: UserFormValues, userId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Failed to update user database record.',
      });
      throw error; 
    }
  };
           ...
           <TooltipProvider>
            <div className="mx-auto max-w-4xl space-y-6">
                 <Alert variant="destructive">
                    ...
                    <AlertTitle>Static Build Notice</AlertTitle>
                    <AlertDescription>
                       In the Android/Static version, creating Auth accounts requires a Cloud Function or separate admin portal. You can manage <b>Firestore user records</b> here, but actual login accounts must be registered via the Firebase Console or Registration Link.
                    </AlertDescription>
                 </Alert>
              <Card>
                <CardHeader>
    ...
    {userToDelete && (
      <AlertDialog open={!!userToDelete} onOpenChange={(open) => !open && setUserToDelete(null)}>
        <AlertDialogContent>
            ...
            <AlertDialogDescription>
                This will delete the user {userToDelete.name} from the database. Note: This does NOT delete the actual login account. You must do that in the Firebase Console.
            </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
