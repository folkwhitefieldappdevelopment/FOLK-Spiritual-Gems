...
import type { Person, Group, FolkStage, CustomField } from "@/lib/types";
...
const ContactsPageComponent = () => {
  ...
  const [importResult, setImportResult] = React.useState<{ successCount: number; errors: any[] } | null>(null);
  ...
  const fetchGroups = React.useCallback(async () => {
    ...
    try {
      ...
      const mineStatic = allStatic.filter(g => {
        const creatorNameLower = (g.createdByName || '').toLowerCase().trim();
        const assigneeNameLower = (g.assignedToName || '').toLowerCase().trim();
        return g.createdBy === myId || 
               creatorNameLower === myNameLower ||
               assigneeNameLower === myNameLower ||
               (g.sharedWithUserIds && g.sharedWithUserIds.includes(myId));
      });
      
      const othersStatic = allStatic.filter(g => {
        const creatorNameLower = (g.createdByName || '').toLowerCase().trim();
        const assigneeNameLower = (g.assignedToName || '').toLowerCase().trim();
        const isMine = g.createdBy === myId || 
                        creatorNameLower === myNameLower ||
                        assigneeNameLower === myNameLower ||
                        (g.sharedWithUserIds && g.sharedWithUserIds.includes(myId));
        return !isMine;
      ...
      const { people: allPeopleForCount } = await getPeople(appUser, { 
        scope: 'all', 
        ignoreLimit: true 
      });
  ...
  const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
        try {
            ...
            const result = await importPeople(mappedData, appUser, (curr, total) => {
                setImportProgress({ current: curr, total });
            });
            ...
            if (result.errors.length === 0) {
                toast({ title: "Import Successful", description: `${result.successCount} contacts synchronized.` });
            } else {
                toast({ variant: 'destructive', title: "Import Completed with Issues", description: `${result.successCount} added, ${result.errors.length} failed. Please review reports.` });
            }
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-[#11121d]">
      ...
      <div className="flex flex-col sm:gap-4 sm:py-4 sm:pl-14">
          <PageHeader title="Outreach" description={totalCount !== null ? `Managing ${totalCount} records in this view.` : "Outreach management."}>
            <div className="flex items-center gap-1.5 py-1">
                <Button variant="outline" size="sm" onClick={() => isGroupTab ? fetchGroups() : fetchContacts(undefined, true)} disabled={isLoading || isLoadingGroups} className="h-9 font-bold px-2.5 rounded-xl border-2 border-white/10 bg-white/5 text-white">
                  ...
                  <>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="sm" className="h-9 font-bold px-2.5 rounded-xl border-2 border-white/10 bg-white/5 text-white"><FileSpreadsheet className="h-4 w-4 sm:mr-2" /> <span className="hidden xs:inline">Data</span></Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="bg-[#1e1e2e] border-white/5 text-white">
                            ...
                            <DropdownMenuItem onSelect={() => exportContactsToExcel(people, "SG_Export", myGroups)} className="font-bold"><Download className="mr-2 h-4 w-4" /> Export Full List</DropdownMenuItem>
                ...
                <Button size="icon" onClick={() => setIsAddMethodDialogOpen(true)} className="h-9 w-9 bg-primary hover:bg-primary/90 rounded-full shadow-lg ml-2"><Plus className="h-5 w-5 text-white" /></Button>
            </div>
          ...
          <main className="flex-1 p-4 sm:px-6 space-y-6 pb-20">
            <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); }} className="w-full">
                <div className="overflow-x-auto scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
                    <TabsList className="mb-8 flex w-max sm:w-full h-auto p-1.5 bg-[#1b1d32] border-none rounded-2xl gap-1.5 min-w-full">
                        <TabsTrigger value="my-contacts" className="flex-1 py-3 px-8 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white data-[state=inactive]:text-slate-500 whitespace-nowrap uppercase">CONTACTS (MINE)</TabsTrigger>
                        {isPrivileged && <TabsTrigger value="all-contacts" className="flex-1 py-3 px-8 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white data-[state=inactive]:text-slate-500 whitespace-nowrap uppercase">CONTACTS (ALL)</TabsTrigger>}
                        <TabsTrigger value="my-groups" className="flex-1 py-3 px-8 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white data-[state=inactive]:text-slate-500 whitespace-nowrap uppercase">GROUPS (MINE)</TabsTrigger>
                        <TabsTrigger value="all-groups" className="flex-1 py-3 px-8 text-[9px] sm:text-[10px] font-black uppercase tracking-widest rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white data-[state=inactive]:text-slate-500 whitespace-nowrap uppercase">GROUPS (SHARED)</TabsTrigger>
                    </TabsList>
                </div>
                ...
                {!isGroupTab && (
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col md:flex-row items-center gap-4 mb-2">
                            ...
                            <Button 
                                ...
                                className={cn(
                                    "h-14 px-8 w-full md:w-auto rounded-2xl border-white/5 bg-[#1b1d32] text-white hover:bg-white/10 font-black uppercase tracking-widest text-[10px]",
                                    isAdvancedSearchOpen && "bg-primary text-white border-primary"
                                )}
                            >
                                ...
                                <div className="flex flex-wrap items-center gap-2 ml-auto pr-2">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="w-64 max-h-80 overflow-y-auto bg-[#1e1e2e] border-white/10 text-white p-2 rounded-2xl shadow-2xl border-none">
                                            <DropdownMenuItem onSelect={() => setIsGroupDialogOpen(true)} className="font-black text-xs uppercase tracking-tight py-3 px-4 focus:bg-white/10 rounded-xl cursor-pointer">
                                            ...
                                            <ScrollArea className="max-h-60">
                                                {myGroups.filter(g => !g.isDynamic).map(g => (
                                ...
                                <div className="p-6 sm:p-8 space-y-8 sm:space-y-10">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">LOCATION</Label>
                                            <Input placeholder="Search area..." value={filters.location} onChange={e => setFilters(p => ({...p, location: e.target.value}))} className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4 focus-visible:ring-primary" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 flex items-center gap-2"><UserCheck className="h-3 w-3" /> PRIMARY ENABLER</Label>
                                            <Select value={filters.enablerInTouchWith} onValueChange={v => setFilters(p => ({...p, enablerInTouchWith: v}))}>
                                                <SelectTrigger className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4"><SelectValue placeholder="Select coordinator..." /></SelectTrigger>
                                        ...
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 flex items-center gap-2"><Tag className="h-3 w-3" /> CONTACT SOURCE</Label>
                                            <Popover>
                                                <PopoverTrigger asChild>
                                                    <Button variant="outline" className="w-full h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4 justify-between">
                                        ...
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 flex items-center gap-2"><History className="h-3 w-3" /> EVENT NAME (HISTORY)</Label>
                                            <Input placeholder="e.g. Sunday Feast..." value={filters.eventName} onChange={e => setFilters(p => ({...p, eventName: e.target.value}))} className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4 focus-visible:ring-primary" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 flex items-center gap-2"><User className="h-3 w-3" /> CALLER NAME (HISTORY)</Label>
                                            <Input placeholder="Who logged it?..." value={filters.callerName} onChange={e => setFilters(p => ({...p, callerName: e.target.value}))} className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4 focus-visible:ring-primary" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1 flex items-center gap-2"><Calendar className="h-3 w-3" /> CALL DATE FROM</Label>
                                            <Input type="date" value={filters.callDateFrom} onChange={e => setFilters(p => ({...p, callDateFrom: e.target.value}))} className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4 [color-scheme:dark]" />
                                        </div>
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">STAYING WITH</Label>
                                            <Select value={filters.stayingWith} onValueChange={v => setFilters(p => ({...p, stayingWith: v}))}>
                                                <SelectTrigger className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4"><SelectValue placeholder="Staying with..." /></SelectTrigger>
                                                <SelectContent className="bg-[#1e1e2e] border-white/5 text-white">
                                        ...
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">CHANTING ROUNDS</Label>
                                            <Select value={filters.chantingRounds} onValueChange={v => setFilters(p => ({...p, chantingRounds: v}))}>
                                                <SelectTrigger className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4"><SelectValue placeholder="Chanting rounds..." /></SelectTrigger>
                                                <SelectContent className="bg-[#1e1e2e] border-white/5 text-white">
                                        ...
                                        <div className="space-y-2">
                                            <Label className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">CALL STATUS</Label>
                                            <Select value={filters.callStatus} onValueChange={v => setFilters(p => ({...p, callStatus: v}))}>
                                                <SelectTrigger className="h-11 sm:h-12 bg-[#161623] border-none rounded-xl text-white font-bold px-4"><SelectValue placeholder="Status..." /></SelectTrigger>
                                                <SelectContent className="bg-[#1e1e2e] border-white/5 text-white">
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-end items-center gap-4 pt-6 border-t border-white/5">
                                        <Button variant="ghost" onClick={resetFilters} className="w-full sm:w-auto font-black text-slate-500 uppercase text-[9px] sm:text-[10px] tracking-widest hover:bg-white/5 px-8 h-12 rounded-xl">Clear Filters</Button>
                                        <Button onClick={() => { fetchContacts(); }} className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-white font-black uppercase text-[9px] sm:text-[10px] tracking-widest px-12 h-14 rounded-2xl shadow-xl shadow-primary/20">Apply Analysis</Button>
                                    </div>
                                </div>
                            </div>
                        </CollapsibleContent>
                    ...
                    <div className="flex justify-center mb-8">
                        <Button 
                            ...
                            className="h-12 px-8 rounded-2xl border-white/10 bg-white/5 text-white hover:bg-white/10 font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl"
                        >
                ...
                <TabsContent value="my-contacts" className="mt-0 focus-visible:outline-none">
                    ...
                    {hasMore && !isLoading && <div className="flex justify-center pb-12 pt-6"><Button onClick={() => fetchContacts(lastDocId || undefined)} disabled={isLoadingMore} variant="outline" className="font-bold px-8 h-12 rounded-xl border-white/10 text-white bg-white/5 uppercase tracking-widest text-[10px]">Load More Records</Button></div>}
                </TabsContent>
                ...
                <TabsContent value="my-groups" className="mt-0 focus-visible:outline-none">
                    <div className="relative mb-6">
                        ...
                        <Input 
                            ...
                            onChange={(e) => {
                                ...
                                setMyGroups(prev => prev.filter(g => g.name.toLowerCase().includes(val)));
                                if (!val) fetchGroups();
                            }}
                        />
                    </div>
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            {myGroups.map(group => (
                                <GroupCard 
                                    key={group.id} 
                                    group={group} 
                                    onEdit={() => { setEditingGroup(group); setIsGroupDialogOpen(true); }}
                                    onDelete={() => deleteGroup(group.id, appUser!).then(() => fetchGroups())}
                                    onStartCall={() => { setEditingGroup(group); setIsConfirmSessionDialogOpen(true); }}
                                />
                            ))}
                        </div>
                    )}
                ...
                <TabsContent value="all-groups" className="mt-0 focus-visible:outline-none">
                     <div className="relative mb-6">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500" />
                        <Input 
                            ...
                            className="h-14 pl-12 rounded-2xl bg-[#1b1d32] border-none text-white font-bold shadow-2xl focus-visible:ring-primary"
                             onChange={(e) => {
                                const val = e.target.value.toLowerCase();
                                setOthersGroups(prev => prev.filter(g => g.name.toLowerCase().includes(val)));
                                if (!val) fetchGroups();
                            }}
                        />
                    </div>
      ...
      <CreateUpdateGroupDialog isOpen={isGroupDialogOpen} setIsOpen={setIsGroupDialogOpen} group={editingGroup} onSave={async (d) => { editingGroup ? await updateGroupSvc(editingGroup.id, d, appUser!) : await createGroup(d, appUser!); fetchGroups(); }} />
...
export default function ContactsPage() { return <React.Suspense fallback={<div className="flex h-screen w-full items-center justify-center bg-[#11121d]"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>}><ContactsPageComponent /></React.Suspense>; }